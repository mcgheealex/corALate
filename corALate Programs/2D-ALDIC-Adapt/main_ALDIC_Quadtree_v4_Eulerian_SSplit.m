...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ImgSeqNum = 3 :  length(ImgNormalized) 
...
%     subplot(2,2,4); imshow(gNormalizedMask'); title('g mask')
    ...
    DICpara.DispSmoothness = 0.0 ; DICpara.StrainSmoothness = 0.01;
...
open(v);
figure,
for ImgSeqNum =   (1+size(ResultDisp,1))-40
    
    
    clf; 
    patch('Faces', ResultFEMeshEachFrame{ImgSeqNum-1}.elementsFEM(:,1:4), ...
        'Vertices', [ResultFEMeshEachFrame{ImgSeqNum-1}.coordinatesFEM(:,1), ...
        DICpara.ImgSize(2)+1-ResultFEMeshEachFrame{ImgSeqNum-1}.coordinatesFEM(:,2)] + ...
        [ResultDisp{ImgSeqNum-1}.U(1:2:end), -ResultDisp{ImgSeqNum-1}.U(2:2:end)], 'Facecolor','none','linewidth',1)
    
    %patch('Faces', DICmesh.elementsFEM(:,1:4), 'Vertices', DICmesh.coordinatesFEMWorld + ...
    %    [ResultDisp{ImgSeqNum-1}.U(1:2:end), -ResultDisp{ImgSeqNum-1}.U(2:2:end)], 'Facecolor','none','linewidth',1)
