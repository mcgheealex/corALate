...
% R_first_collapse_loc = 32; % unit: #

%%%%%% 20_02_48 %%%%%%
load('R_data.mat')

load('GE_RofTdata_RtFit.mat');
% R_exp([2:7]) = []; % unit: m
...
% ====== Read images ====== 
[file_name_temp ,Img_temp ,DICpara] = ReadImageQuadtree; % Load DIC raw images
DICpara.ImgSeqIncUnit = 1;
DICpara.winsizeMin = 4; % min element size in the refined quadtree mesh
    
% ====== Load mask files ======
[mask_file_name_temp ,ImgMask_temp ] = ReadImageMasks;  

%%%%%% cav-dic: reverse image order %%%%%%
Img = [  Img_temp , Img_temp(:,1) ];
file_name = [ file_name_temp, file_name_temp(:,1)];
ImgMask = [ ImgMask_temp, ImgMask_temp(:,1) ];
mask_file_name = [ mask_file_name_temp, mask_file_name_temp(:,1) ];

clear file_name_temp Img_temp mask_file_name_temp ImgMask_temp
...
%     Plotstrain_show(FSubpb2World,DICmesh.coordinatesFEMWorld,DICmesh.elementsFEM(:,1:4),DICpara,'EdgeColor');
    ...
    UpdateY = 1e4;  
    HPar = cell(21,1); 
    for tempj = 1:21, HPar{tempj} = HtempPar(:,tempj); end
...
coordRef = [tempx,tempy]; coordCurr = coordRef;  
...
for ImgSeqNum = [2 : length(ImgNormalized)]
    ...
    if ImgSeqNum == R_forward_stop+1 % Start to backward calculating the disp field
        coordCurr = coordRef;
    end
    if ImgSeqNum < R_forward_stop+1  % Forward from the beginning
        ImgSeqNum_hybrid = ImgSeqNum; 
    else % Backward from the end
        ImgSeqNum_hybrid = length(ImgNormalized)-(ImgSeqNum-(R_forward_stop+1)); 
    end
         
    tempx = ResultFEMeshEachFrame{ImgSeqNum_hybrid-1}.coordinatesFEM(:,1);
    tempy = ResultFEMeshEachFrame{ImgSeqNum_hybrid-1}.coordinatesFEM(:,2);
    
    tempu = ResultDisp{ImgSeqNum_hybrid-1}.U(1:2:end);
    tempv = ResultDisp{ImgSeqNum_hybrid-1}.U(2:2:end);
   
    if ImgSeqNum > R_forward_stop % Backward from the end
        tempx = tempx + tempu; tempy = tempy + tempv;
        tempu = -tempu; tempv = -tempv;
    end
    ...
    U_cum = (coordCurr - coordRef)'; U_cum = U_cum(:); 
    ResultDisp{ImgSeqNum_hybrid-1}.U_cum_store = U_cum; % Store cumulative displacement field
    
end

close(hbar);

Plotdisp_show( ResultDisp{50-1}.U_cum_store,ResultFEMeshEachFrame{1}.coordinatesFEM,ResultFEMeshEachFrame{1}.elementsFEM(:,1:4),DICpara,'EdgeColor');
...
for ImgSeqNum = [ 50 ] % 2 : 10 :  length(ImgNormalized)-1 ] 
    ...
    DICpara.DoYouWantToSmoothOnceMore = 1; % Smooth strain fields if necessary
    %prompt = 'Do you want to smooth displacement? (0-yes; 1-no)';
    %DoYouWantToSmoothOnceMore = input(prompt); 
    SmoothTimes = 0;
    try
  ...
  SaveFigFilesDispAndStrainQuadtree;
   % pause;
    ...
    ResultCavDIC{ImgSeqNum-1} = struct('bubble_center_x',bubble_x,'bubble_center_y',bubble_y, ...
            ...
            'R',Rnew(ImgSeqNum-1)*DICpara.um2px,'DICwinsizePhy',DICpara.winsize*DICpara.um2px,  ...
...
for ImgSeqNum = 2 : (1+size(ResultDisp,1))
    ...
    clf; patch('Faces', ResultFEMeshEachFrame{ImgSeqNum}.elementsFEM(:,1:4), ...
        'Vertices', ResultFEMeshEachFrame{ImgSeqNum}.coordinatesFEM  + ...
        [ResultDisp{ImgSeqNum}.U(1:2:end),  ResultDisp{ImgSeqNum}.U(2:2:end)], 'Facecolor','none','linewidth',1)
