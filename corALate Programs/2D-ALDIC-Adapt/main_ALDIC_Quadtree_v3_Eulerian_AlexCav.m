...
addpath('./func_quadtree/rbfinterp');
...
for ImgSeqNum = 2 : length(ImgNormalized) 
    ...
    [USubpb1_Eulerian,FSubpb1_Eulerian,HtempPar,ALSub1Timetemp,ConvItPerEletemp,LocalICGNBadPtNumtemp,markCoordHoleStrain] = ...
        LocalICGNQuadtree(U0,DICmesh.coordinatesFEM_Eulerian,Dg,gNormalized,fNormalized,DICpara,'GaussNewton',tol);
...
% 
%         [strain_exx,strain_exy,strain_eyy,strain_principal_max,strain_principal_min, ...
%             strain_maxshear,strain_vonMises] = PlotstrainQuadtreePolarMasks(UWorld,FWorld, ...
%             coordinatesFEMWorld,elementsFEM(:,1:4),file_name{1, ImgSeqNum}, ...
%             ImgMask{ ImgSeqNum },DICpara, [] ,ImgSeqNum);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         % load('D:\JinYang\MATLAB\2D_ALDIC_v4.2\img_cav_70121_DICpaper_150um_Top_2Mfps_1\R_data.mat')
         ...
         ImgSeqNum_skip_start = 2; ImgSeqNum_skip_end = 255;
         Rnew(ImgSeqNum_skip_start:ImgSeqNum_skip_end) = [];
         ...
         [row0,col0] = find(abs(theta-0.5*pi)<45/180*pi &  r> (Rnew(ImgSeqNum)+DICpara.winsize)*DICpara.um2px);
         clf,  
         subplot(2,2,1), plot([2:1:length(ImgNormalized)], Rnew(2:length(ImgNormalized))*DICpara.um2px,'s-');
         hold on; plot(ImgSeqNum, Rnew(ImgSeqNum)*DICpara.um2px, 'rs','MarkerFaceColor','r');
         axis([2,255,0,140]);
         title(['Frame #',num2str(ImgSeqNum)],'fontweight','normal','interpreter','latex')
         ...
         figure, plot(r(row0)/DICpara.um2px,disp_r(row0)/DICpara.um2px,'.')
         ...
         subplot(2,2,2), plot(r(row0)/(max(Rnew)*DICpara.um2px),disp_r(row0),'.');
         
         %  title(['Frame #',num2str(ImgSeqNum)],'fontweight','normal' )
         xlabel('$r/R_{\rm{max}}$','interpreter','latex');
         ylabel('$u_r$ ($\mu$m)','interpreter','latex');
         set(gca,'fontsize',20);  
         axis([0.8,4, 0, 25 ]);
         a = gca; a.TickLabelInterpreter = 'latex';
...
%          writeVideo(v,frame);
         ...
         dudx = FWorld(1:4:end); dvdx = FWorld(2:4:end);
         dudy = FWorld(3:4:end); dvdy = FWorld(4:4:end);
         
         strain_err = dudx.*c2 + dudy.*cs + dvdx.*cs + dvdy.*s2;
         strain_ert = 0.5*(  -dudx.*cs + dudy.*c2 - dvdx.*s2 + dvdy.*cs + ...
             -dudx.*cs - dudy.*s2 + dvdx.*c2 + dvdy.*cs );
         strain_ett = dudx.*s2 - dudy.*cs - dvdx.*cs + dvdy.*c2;
         ...
         Jacobian = (1+strain_err).*(1+strain_ett).^2;
          ...
          %%%%% Log Err %%%%%
         subplot(2,2,3),  plot(r(row0)/(max(Rnew)*DICpara.um2px),strain_logErr(row0),'.');
         %title(['Frame #',num2str(ImgSeqNum)],'fontweight','normal' )
         xlabel('$r/R_{\rm{max}}$','interpreter','latex');
         ylabel('Log strain $E_{rr}$','interpreter','latex');
         set(gca,'fontsize',20);  
           axis([0.8,4,1e-4,1]);
         ...
         %%%%% Volume change Jacobian %%%%%
         subplot(2,2,4), plot(r(row0)/(max(Rnew)*DICpara.um2px),Jacobian(row0),'.');
         %title(['Frame #',num2str(ImgSeqNum)],'fontweight','normal' )
         xlabel('$r/R_{\rm{max}}$','interpreter','latex');
         ylabel('det($\mathbf{F}$)','interpreter','latex');
         set(gca,'fontsize',20);  
         axis([0.8,4,0.95,1.2]);
         a = gca; a.TickLabelInterpreter = 'latex';
