...
% for GUI version
%[file_name,Img,DICpara] = func_GetImages; % Load DIC raw images
...
for ImgSeqNum = 2 : length(ImgNormalized)  
    ...
    % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    mu=0; beta=0; tol=1e-2;
    
    % ====== ALStep 1 Subproblem1: Local Subset DIC ======
    ALSolveStep=1; ALSub1Time=zeros(6,1); ALSub2Time=zeros(6,1); 
...
% ------ Plot ------
USubpb2World = USubpb2; 
USubpb2World(2:2:end) = -USubpb2(2:2:end); 
FSubpb2World = FSubpb2; 
FSubpb2World(2:4:end) = -FSubpb2World(2:4:end); 
FSubpb2World(3:4:end) = -FSubpb2World(3:4:end); 
close all; 
