...
% This is the only line needs to update to process multi-frame tiff images.
run_read_tiff_stack;
...
if length(ImgNormalized)==2, DICpara.Image2PlotResults = 1; % funParaInput('Image2PlotResults');
else DICpara.Image2PlotResults = 1; % Plot over current, deformed image by default
end
% ------ Save fig format ------
DICpara.MethodToSaveFig = 1; % funParaInput('SaveFigFormat');
% ------ Choose overlay image transparency ------
DICpara.OrigDICImgTransparency = 1;
if DICpara.MethodToSaveFig == 1
    DICpara.OrigDICImgTransparency = 1; % funParaInput('OrigDICImgTransparency');
end
