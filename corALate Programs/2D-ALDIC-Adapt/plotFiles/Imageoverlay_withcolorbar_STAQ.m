function [Imgout] = Imageoverlay_withcolorbar_STAQ(BG, OL, Mask, alpha, PlotTitle, scaleBar , colorScale)
    ...
    sidepadding = 10;
    toppadding = 0;
    CBpadding = round(0.1*rowsa);
    Rez = 7;
    BS = round((rowsa - CBpadding)/(Rez+1)) ;
    ws = 3*BS; % how much white space to add to the edge
    fontsize = round(BS/2);
    ...
    totLen = rowsa-BS*1.3; % Rez*(BS+sidepadding); % this is the total length of the colorbar
